...
onMounted(() => {
  // Only initialize if Office.js is ready or we're not in Office environment
  if (!$officeState || $officeState.isReady) {
    // In token-based auth, we don't have persistent sessions
    // User needs to authenticate each time they open the add-in
    // This is acceptable for Office Add-ins due to security constraints
  }
})
</script>
